...
void ardHdlc_escape_value(char value, char *dest, int *dest_index) {
...
ardHdlc_control_t ardHdlc_parseControlByte(unsigned char control) {
  ...
  if (control & (1 << ARDHDLC_CONTROL_S_OR_U_FRAME_BIT)) {
    ...
    if (((control >> ARDHDLC_CONTROL_S_FRAME_TYPE_BIT) & 0x3)
        == ARDHDLC_CONTROL_TYPE_RECEIVE_READY) {
...
int ardHdlc_decodeFrame(ardHdlc_state_t *state, ardHdlc_control_t *control, const char *src,
  ...
  // A frame is at least 6 bytes in size
  else if (state->end_index < state->start_index + 5){
...
int ardHdlc_createDataFrame(ardHdlc_control_t *control, const char *src,
  ...
  ardHdlc_escape_value(ARDHDLC_ALL_STATION_ADDR, dest, &dest_index);

  // Add the framed control field value
  value = ardHdlc_buildControlByte(control);
  fcs = fcs16(fcs, value);
  ardHdlc_escape_value(value, dest, &dest_index);
  ...
  if (control->frame == ARDHDLC_FRAME_DATA) {
    ...
    for (i = 0; i < src_len; i++) {
      fcs = fcs16(fcs, src[i]);
      ardHdlc_escape_value(src[i], dest, &dest_index);
    }
  }
