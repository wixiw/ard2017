...
void Hdlc::receiveNextByte(uint8_t data)
{
    /* FRAME FLAG */
    if(data == FRAME_BOUNDARY_OCTET)
    {
        ...
        {
            //TODO il faudrait une erreur ici
            this->escape_character = false;
        }
                  ...
                  ( this->frame_checksum == ((this->receive_frame_buffer[this->frame_position-1] << 8 ) | (this->receive_frame_buffer[this->frame_position-2] & 0xff)) ) )  // (msb << 8 ) | (lsb & 0xff)
        {
            ...
            handleFrame(receive_frame_buffer,(uint8_t)(this->frame_position-2));
        }//TODO il faudrait mettre des erreurs ici
        this->frame_position = 0;
        this->frame_checksum = CRC16_CCITT_INIT_VAL;
        return;
    }
    ...
    if(this->frame_position-2 >= 0) {
        this->frame_checksum = update_crc_ccitt(this->frame_checksum, receive_frame_buffer[this->frame_position-2]);
    }
    ...
    {
        //TODO il faudrait mettre une erreur ici
        this->frame_position = 0;
        this->frame_checksum = CRC16_CCITT_INIT_VAL;
    }
}
...
void Hdlc::encodeFrame(const char *framebuffer, uint8_t frame_length)
{
    uint8_t data;
    uint16_t fcs = CRC16_CCITT_INIT_VAL;

    this->sendByte((uint8_t)FRAME_BOUNDARY_OCTET);

    while(frame_length)
    {
        data = *framebuffer++;
        fcs = update_crc_ccitt(fcs, data);
        if((data == CONTROL_ESCAPE_OCTET) || (data == FRAME_BOUNDARY_OCTET))
        {
            this->sendByte((uint8_t)CONTROL_ESCAPE_OCTET);
            data ^= INVERT_OCTET;
        }
        this->sendByte((uint8_t)data);
        frame_length--;
    }
    data = low(fcs);
    if((data == CONTROL_ESCAPE_OCTET) || (data == FRAME_BOUNDARY_OCTET))
    ...
    if((data == CONTROL_ESCAPE_OCTET) || (data == FRAME_BOUNDARY_OCTET))
    {
        this->sendByte(CONTROL_ESCAPE_OCTET);
        data ^= INVERT_OCTET;
    }
    this->sendByte(data);
    this->sendByte(FRAME_BOUNDARY_OCTET);
}
