 ...
 */
void ObjectColor::computeFromRawData(uint16_t red, uint16_t green, uint16_t blue, uint16_t lux)
{
    ...
    if (100 <= r && r <= 135 && 93 <= g && g <= 109 && 38 <= b && b <= 50)
        c = eObjectColor_YELLOW;
    else if (34 <= r && r <= 82 && 83 <= g && g <= 90 && 105 <= b && b <= 135)
        c = eObjectColor_BLUE;
    else
...
String ObjectColor::toString()
{
    String color = "";
    switch (c)
    {
    case (eObjectColor_UNDEF):
        color = "UNDEFINED";
        break;
    case (eObjectColor_BLUE):
        color = "BLUE";
        break;
    case (eObjectColor_YELLOW):
        color = "YELLOW";
        break;
    case (eObjectColor_WHITE):
        color = "WHITE";
        break;
    default:
        ;
...
{
    state = apb_ColorSensorState_init_default;
}
...
void ColorSensor::update(TimeMs sinceLastCall)
{
...
apb_ColorSensorState const & ColorSensor::getState()
{
    state = apb_ColorSensorState_init_default;
    state.r = color.r;
    state.g = color.g;
