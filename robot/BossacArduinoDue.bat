...
REM add -v arg to verify flash after upload
ECHO Execute bossac with command line:
ECHO "%BOSSACPATH%" --port=%COMPORT% -U false -e -w -b -R "%BINFILE%"
START /WAIT "" "%BOSSACPATH%" --port=%COMPORT% -U false -e -w -b -R "%BINFILE%"
